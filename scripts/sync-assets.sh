...
$(cat "$NOTIFY_FILE")"
  ...
  echo '=== Uploading Assets to Release Channel ==='
  if [[ ! -f $ASSETS_FILE ]]; then
    echo "Creating new release for $PROFILE first ..."
    gh release create "$PROFILE" --title "$PROFILE" --repo "$GITHUB_REPOSITORY"
  fi

  gh release upload "$PROFILE" "$REPO_DIR"/* --clobber --repo "$GITHUB_REPOSITORY"
else
  echo '=== No database events, skipping upload. ==='
fi

# Remove release assets no longer in the database, typically after a package is updated.
# Do this after the new assets are uploaded successfully.
if [[ -s $OBSOLETE_FILE ]]; then
  ...
  while read -r asset || [[ -n $asset ]]; do
    ...
    gh release delete-asset "$PROFILE" "$asset" -y --repo "$GITHUB_REPOSITORY" || echo "Failed to delete asset: $asset"
  done < "$OBSOLETE_FILE"
else
